...
{	int kern_ret = 0;
	if(!userInput.list && !userInput.modify && !userInput.new_priority ){
		...
		if (nls_ret.type == 0) {
			printf("\nCALL BACK interrupt in Child thread: Job id %d completed. ", nls_ret.id);
			printf("Job type: ");
			...
			if ((int)*(nls_ret.result) < 0) {
				printf(" status: failed, error : %s  ", strerror(-(int)*(nls_ret.result)));
			}
			else
			{
	...
	while ((input_arg = getopt(argc, argv, "edcuslfwr:m:n:a:p:h")) != -1)
	{
		switch (input_arg)
		{
			...
			case 'h': // help
				...
				printf("-e = encrypt, -d = decrypt, -c = compress, -u = uncompress, -s = checksum, -l = list, -f = flush all jobs, -r = remove a job -m = modify job priority, -n = new priority with -m, -w= weak validation during submission\n");
...
int main(int argc, char *argv[])
{
	...
	// do some work here while thread1 listens for netlink message
	while (!message_recieved) {
		printf("%s (Process Id %d) :: I am doing something useful..\n", argv[0], getpid());
		sleep(1);
	}
	
	printf("%s (Process Id %d) :: I can still continue doing work..\n", argv[0], getpid());
	int time = 10;
	while (time) {
		time--;
		sleep(1);
		printf("%s (Process Id %d) :: More Work.. \n", argv[0], getpid());
	}
